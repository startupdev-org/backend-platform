...
- `LocationService` and `EmployeeLocationServicePriceService` have **no ownership check** — unlike every other service in this codebase, they never verify that the target `Business`/`Employee`/`Location`/`ProvidedService` belongs to the calling user. Fix before layering billing/limits on top of this, or a careless or malicious tenant can attach resources to — or read/delete resources from — another tenant's business.
- `LocationService` throws raw `RuntimeException` instead of `ResourceNotFoundException`/`BusinessException` — breaks the `GlobalExceptionHandler` contract documented above; a missing location currently likely surfaces as a 500, not a 404.
